...
;
; CALLING SEQUENCE:  THEMIS_IMAGER_READFILE,Filename,Images,Stride,[Metadata]
;
...
PRO THEMIS_IMAGER_READFILE_NEW,filename,images,metadata,COUNT=n_frames,DEBUG=debug,ALL_METADATA=all_metadata,STRIDE=stride
  ...
  FOR indx=0,nfiles-1 DO BEGIN
    ...
    WHILE NOT EOF(lun) DO BEGIN
     ...
     ; IF THEMIS_IMAGER_PNM_READFILE(lun,image,comments,DEBUG=debug) THEN BREAK
      image= PAM_READFILE_NEW(LUN=lun,COMMENTS=comments,DEBUG=debug); NEEDED TO CHANGE THE PAM READFILE TO *_NEW
