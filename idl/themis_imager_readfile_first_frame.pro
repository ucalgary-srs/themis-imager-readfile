...
FUNCTION THEMIS_IMAGER_PNM_READFILE,lun,image,comments,DEBUG=debug
  ...
  RETURN,0
  ioerror:PRINT,!ERROR_STATE.MSG & RETURN,1
END
...
PRO THEMIS_IMAGER_READFILE_FIRST_FRAME,filename,images,metadata,COUNT=n_frames,DEBUG=debug,ALL_METADATA=all_metadata
  ON_ERROR,0
 ...
 ;pre-allocating memory significantly increases speed
  ...
  FOR indx=0,nfiles-1 DO BEGIN
   ...
   WHILE NOT EOF(lun) DO BEGIN
     ...
     ; images[0,n_frames]=image

	  n_frames= n_frames+1

      BREAK  ;break out after the first frame
